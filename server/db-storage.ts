...
export class DatabaseStorage {
  ...
  private initializeDefaultData() {
    ...
    // Add default wallet addresses for crypto deposits/withdrawals
    this.walletAddresses.push(
      ...
      },
      {
        id: randomUUID(),
        currency: "LTC",
        address: "LTC1qw508d6qejxtdg4y5r3zarvary0c5xw7k2h5z0f",
  ...
  async getAllWalletAddresses(): Promise<any[]> {
    return this.walletAddresses.filter(wallet => wallet.isActive);
  }

  async getWalletAddress(currency: string): Promise<any | undefined> {
    return this.walletAddresses.find(wallet => wallet.currency === currency && wallet.isActive);
  }

  async updateWalletAddress(currency: string, address: string, network: string, updatedBy: string): Promise<any | undefined> {
    const existingWallet = this.walletAddresses.find(w => w.currency === currency);
    if (!existingWallet) return undefined;

    existingWallet.address = address;
    existingWallet.network = network;
    existingWallet.updatedBy = updatedBy;
